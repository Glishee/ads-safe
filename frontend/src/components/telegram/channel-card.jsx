...
export default function ChannelCard({ channel, onCardClick }) {
  const { language } = useLanguage();
  const navigate = useNavigate();

  const handleRequestAd = (e) => {
    e.stopPropagation();
  ...
  const handleViewChannel = (e) => {
    e.stopPropagation();
    window.open(channel.telegram_link, "_blank");
  };
  ...
  return (
    <Card
      className="overflow-hidden transition-all duration-200 hover:shadow-lg hover:-translate-y-1 flex flex-col h-full cursor-pointer"
    ...
    >
      <div className="relative h-32 bg-gradient-to-r from-blue-400 to-purple-500">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/70 to-transparent">
          <h3 className="text-lg font-bold text-white truncate" title={channel.name}>
            {channel.name}
          </h3>
        </div>
      </div>
